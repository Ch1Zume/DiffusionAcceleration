...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        save_image(images[0], '/work/SJTU/flow_grpo/test_outputs/eval_output.png')
...
def main(_):
    ...
    image = pipeline(prompt, height=512, width=512, num_inference_steps=40,guidance_scale=4.5,negative_prompt="").images[0]
    image.save(f"test_outputs/flow_grpo.png")
