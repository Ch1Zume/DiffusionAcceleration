            ...
            for i, train_sample_batch in tqdm(
                ...
                # Forward Rollout
                    ...
                    with torch_autocast(enabled=enable_amp, dtype=mixed_precision_dtype):
                        ...
                        transformer_ddp.module.set_adapter("default")
                        # updating policy + x_t --> forward prediction
                        ...
                        # Reference model: same Transformer with LoRA disabled
                        with torch.no_grad():  
                    ...
                    # for a positive sample, its update: forward prediction is effective, so we incorporate it 
                    # it also has an EMA sense to consider old prediction to make training stable
                    # pos pred - old pred = beta (forward pred - old pred), OP = beta * OF
                    positive_prediction = config.beta * forward_prediction + (1 - config.beta) * old_prediction.detach()
                    # for a negative sample, we do not want its update
                    # we try to keep the old policy, and subtract the effect of this negative sample
                    # neg pred - old pred = beta(old pred - forward pred), ON = beta * FO
                    implicit_negative_prediction = (
                        1.0 + config.beta
                    ) * old_prediction.detach() - config.beta * forward_prediction

                    # adaptive weighting
                    # recompute the clean image
