...
from diffusers.models.activations import FP32SiLU, GELU, GEGLU, ApproximateGELU, SwiGLU, LinearActivation
from diffusers.models.attention_processor import Attention, JointAttnProcessor2_0
from diffusers.models.embeddings import SinusoidalPositionalEmbedding
from diffusers.models.normalization import SD35AdaLayerNormZeroX, AdaLayerNormZero, AdaLayerNormContinuous, \
    AdaLayerNorm, RMSNorm
from diffusers.utils import logging, deprecate
from diffusers.utils.torch_utils import maybe_allow_in_graph
from torch import nn
from .ts_utils import forecast, derivative_approximation, taylor_formula
...
# 能够调用任意阶Taylor
@maybe_allow_in_graph
class JTBlock_HighTaylor(nn.Module):
    r"""
    A Transformer block following the MMDiT architecture, introduced in Stable Diffusion 3.
    ...
    def forward(
        ...
        if use_cache:
            if action == 0: # full compute
                ...
                # 键值对的创建在 derivative_approximation 中完成
                derivative_approximation(
                    cache_dict = cache_dict,
                    feature = attn_output.detach(),
                    module = 'attn',
                    max_order = min(t, status['max_order']), # 这里其实已经限制了全计算步与阶数的关系了
                    layer_idx = layer_idx,
                ...
                derivative_approximation(
                    cache_dict = cache_dict,
                    feature = context_attn_output.detach(),
                    module = 'context_attn',
                    max_order = min(t, status['max_order']),
                    layer_idx = layer_idx,
                    interval = status['interval'],
                    step = t,
                    )
            else: # Using Taylor Cache
                attn_output = taylor_formula(
                    cache_dict = cache_dict,
                    distance = status['distance'],
                    module = 'attn',
                    layer_idx = layer_idx,
                    last_full_step = status['last_full_compute'],
                    )

                context_attn_output = taylor_formula(
                    cache_dict = cache_dict,
                    distance = status['distance'],
                    module = 'context_attn',
                    layer_idx = layer_idx,
